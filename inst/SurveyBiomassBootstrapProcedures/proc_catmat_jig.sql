...
IF EXISTS (
  SELECT * 
   ...
   WHERE SPECIFIC_SCHEMA = N'dbo'
     AND SPECIFIC_NAME = N'proc_catmat_jig'
)
   DROP PROCEDURE proc_catmat_jig
GO

CREATE PROCEDURE proc_catmat_jig @sid INTEGER, @spp VARCHAR(3)
AS
   SET NOCOUNT ON
   ...
   SELECT S.SURVEY_SERIES_ID,
      ...
      DATEDIFF(minute, FE_END_DEPLOYMENT_TIME, FE_BEGIN_RETRIEVAL_TIME) / 60.0 AS EFFORT_HRS,
      ...
      ISNULL(HS.HLNSP_HOOK_COUNT, HS.HLNSP_HOOKS_SET_COUNT) AS HOOK_COUNT
   INTO #SETS
   FROM SURVEY S
   ...
   WHERE S.SURVEY_ID = @sid AND ISNULL(USABILITY_CODE,1) IN (0, 1, 2, 6, 12) AND FE.FE_PARENT_EVENT_ID IS NULL AND
      ISNULL(HS.HLNSP_HOOK_COUNT, HS.HLNSP_HOOKS_SET_COUNT) IS NOT NULL AND
      DATEDIFF(minute, FE_END_DEPLOYMENT_TIME, FE_BEGIN_RETRIEVAL_TIME) > 0
   ...
   SELECT S.SURVEY_SERIES_ID,
      ...
      S.HOOK_COUNT,
      C.CATCH_COUNT,
      C.CATCH_COUNT / (S.HOOK_COUNT * S.EFFORT_HRS) AS CPUE_PPH
   FROM #SETS S
      INNER JOIN #CATCH C ON
