SELECT TRIP_START_DATE,
  CASE WHEN ISNULL(SM.FE_END_DEPLOYMENT_TIME, SM.FE_BEGIN_RETRIEVAL_TIME, SM.FE_BEGIN_DEPLOYMENT_TIME, SM.FE_END_RETRIEVAL_TIME, SM.SAMPLE_DATE) <= TRIP_END_DATE THEN
    MONTH(ISNULL(SM.FE_END_DEPLOYMENT_TIME, SM.FE_BEGIN_RETRIEVAL_TIME, SM.FE_BEGIN_DEPLOYMENT_TIME, SM.FE_END_RETRIEVAL_TIME, SM.SAMPLE_DATE)) ELSE
    MONTH(TRIP_END_DATE) AS MONTH,
  CASE WHEN ISNULL(SM.FE_END_DEPLOYMENT_TIME, SM.FE_BEGIN_RETRIEVAL_TIME, SM.FE_BEGIN_DEPLOYMENT_TIME, SM.FE_END_RETRIEVAL_TIME, SM.SAMPLE_DATE) <= TRIP_END_DATE THEN
    DAY(ISNULL(SM.FE_END_DEPLOYMENT_TIME, SM.FE_BEGIN_RETRIEVAL_TIME, SM.FE_BEGIN_DEPLOYMENT_TIME, SM.FE_END_RETRIEVAL_TIME, SM.SAMPLE_DATE)) ELSE
    DAY(TRIP_END_DATE) AS DAY,
  ...
  FE.FE_START_LATTITUDE_DEGREE + FE.FE_START_LATTITUDE_MINUTE / 60 AS Lat_start,
  FE.FE_END_LATTITUDE_DEGREE + FE.FE_END_LATTITUDE_MINUTE / 60 AS Lat_end,
  -SM.Best_Long AS LONGITUDE,
  -(FE.FE_START_LONGITUDE_DEGREE + FE.FE_START_LONGITUDE_MINUTE / 60) AS Lon_start,
  -(FE.FE_END_LONGITUDE_DEGREE + FE.FE_END_LONGITUDE_MINUTE / 60) AS Lon_end,
  SM.Best_Depth,
  SM.GEAR_CODE,
  GEAR_DESC,
  SM.SPECIES_CODE,
...
WHERE SM.FE_PARENT_EVENT_ID IS NULL AND SM.TRIP_SUB_TYPE_CODE NOT IN (2, 3) AND
  ...
  (SPECIES_CATEGORY_CODE IS NULL OR SPECIES_CATEGORY_CODE IN (0, 1, 3, 4, 5, 6)) AND
