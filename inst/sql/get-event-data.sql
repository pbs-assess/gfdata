SELECT
    FISHING_EVENT_ID,
    FE.FE_PARENT_EVENT_ID,
    S.SURVEY_ID,
    S.SURVEY_SERIES_ID,
    -- Get date
    YEAR(FE_BEGIN_RETRIEVAL_TIME) YEAR,
    YEAR(T.TRIP_START_DATE) AS YEAR,
    MONTH(COALESCE (FE_BEGIN_BOTTOM_CONTACT_TIME, FE_END_BOTTOM_CONTACT_TIME, FE_END_DEPLOYMENT_TIME,
      FE_BEGIN_RETRIEVAL_TIME, FE_BEGIN_DEPLOYMENT_TIME, FE_END_RETRIEVAL_TIME)) AS MONTH,
    DAY(COALESCE (FE_BEGIN_BOTTOM_CONTACT_TIME, FE_END_BOTTOM_CONTACT_TIME, FE_END_DEPLOYMENT_TIME,
      FE_BEGIN_RETRIEVAL_TIME, FE_BEGIN_DEPLOYMENT_TIME, FE_END_RETRIEVAL_TIME)) AS DAY,
    -- Get effort (timing, width, speed)
    ISNULL(FE_BEGIN_BOTTOM_CONTACT_TIME, FE_END_DEPLOYMENT_TIME) AS TIME_DEPLOYED,
    ISNULL(FE_END_BOTTOM_CONTACT_TIME, FE_BEGIN_RETRIEVAL_TIME) AS TIME_RETRIEVED,
    ISNULL(DATEDIFF(MI,FE_BEGIN_BOTTOM_CONTACT_TIME, FE_END_BOTTOM_CONTACT_TIME),
         DATEDIFF(MI,FE_END_DEPLOYMENT_TIME,FE_BEGIN_RETRIEVAL_TIME)) AS DURATION_MIN,
    NULLIF(FE_DISTANCE_TRAVELLED,0) * 1000.0 AS TOW_LENGTH_M, -- Convert to meters
    ISNULL(NULLIF(TS.TRLSP_MOUTH_OPENING_WIDTH,0), ISNULL(BD.doorspread,0)) AS MOUTH_WIDTH_M
    ISNULL(NULLIF(TS.TRLSP_DOORSPREAD,0), ISNULL(BD.doorspread,0)) AS DOORSPREAD_M,
    ISNULL(NULLIF(TS.TRLSP_SPEED,0) * 16.66667, ISNULL(speed,0) * 16.66667) AS SPEED_MPM -- 16.66667 is constant to convert from km/hr to m/s
    HOOK_CODE,
    SKATE_COUNT,
    LGLSP_HOOK_COUNT,
    0.0024384 * 0.009144 * LGLSP_HOOK_COUNT AS AREA_SWEPT_KM2
    -- Get location
    FE_START_LATTITUDE_DEGREE + FE_START_LATTITUDE_MINUTE / 60 AS LATITUDE,
    -(FE_START_LONGITUDE_DEGREE + FE_START_LONGITUDE_MINUTE / 60) AS LONGITUDE,
    FE_END_LATTITUDE_DEGREE + FE_END_LATTITUDE_MINUTE / 60 AS LATITUDE_END,
    -(FE_END_LONGITUDE_DEGREE + FE_END_LONGITUDE_MINUTE / 60) ASLONGITUDE_END,
    FE_BEGINNING_BOTTOM_DEPTH AS DEPTH_M
    FE_END_BOTTOM_DEPTH AS DEPTH_END
    FE_BOTTOM_WATER_TEMPERATURE,
    -- Get grouping variables
    T.VESSEL_ID AS VESSEL_ID,
    T.CAPTAIN_ID AS CAPTAIN_ID,
    ISNULL(USABILITY_CODE,1)
    FE.GROUPING_CODE,
    FEG.GROUPING_CODE AS GROUPING_CODE_TRAWL,
    FE.MAJOR_STAT_AREA_CODE,
    FE.MINOR_STAT_AREA_CODE,
    GROUPING_DESC,
    GROUPING_DEPTH_ID,
    FROM FISHING_EVENT FE
      LEFT JOIN TRAWL_SPECS TS ON FE.FISHING_EVENT_ID = TS.FISHING_EVENT_ID
      LEFT JOIN LONGLINE_SPECS LLSP ON LLSP.FISHING_EVENT_ID = FE.FISHING_EVENT_ID
      LEFT JOIN TRIP T ON FE.TRIP_ID = T.TRIP_ID
      LEFT JOIN TRIP_SURVEY TRS ON T.TRIP_ID = TRS.TRIP_ID
      LEFT JOIN SURVEY S ON TRS.SURVEY_ID = S.SURVEY_ID
      LEFT JOIN SURVEY_SERIES SS ON SS.SURVEY_SERIES_ID = S.SURVEY_SERIES_ID
      LEFT JOIN FISHING_EVENT_GROUPING FEG ON FE.FISHING_EVENT_ID = FEG.FISHING_EVENT_ID
      LEFT JOIN SURVEY_GROUPING SG ON S.SURVEY_ID = SG.SURVEY_ID AND  SG.GROUPING_CODE = FEG.GROUPING_CODE
      LEFT JOIN GROUPING G ON G.GROUPING_CODE = FE.GROUPING_CODE
       -- Get mean doorspread and speed from table of defaults
      LEFT JOIN BOOT_DEFAULTS BD ON BD.SURVEY_ID = S.SURVEY_ID
