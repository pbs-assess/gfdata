SELECT TRIP_START_DATE,
 ...
 CASE WHEN SURVEY_SERIES_ALT_DESC IS NULL THEN SURVEY_SERIES_TYPE_ALT_DESC
  ...
  STORAGE_CONTAINER_TYPE AS DNA_SAMPLE_TYPE,
...
FROM GFBioSQL.dbo.SURVEY S
  ...
  LEFT JOIN (SELECT SAMPLE_ID, MIN(SPECIMEN_ID) AS SPECIMEN_ID, SPECIMEN_COLLECTED_IND
	...
	LEFT JOIN (SELECT SAMPLE_ID, MIN(SPECIMEN_ID), COLLECTED_ATTRIBUTE_CODE, STORAGE_CONTAINER_SUB_ID, STORAGE_CONTAINER_TYPE AS SPECIMEN_ID, COLLECTED_ATTRIBUTE_CODE, STORAGE_CONTAINER_SUB_ID, STORAGE_CONTAINER_TYPE
		FROM GFBioSQL.dbo.SPECIMEN_COLLECTED
		WHERE COLLECTED_ATTRIBUTE_CODE IN (4)
		GROUP BY SAMPLE_ID, SPECIMEN_ID, STORAGE_CONTAINER_SUB_ID, STORAGE_CONTAINER_TYPE) SC ON SP.SPECIMEN_ID = SC.SPECIMEN_ID AND SP.SAMPLE_ID = SC.SAMPLE_ID
