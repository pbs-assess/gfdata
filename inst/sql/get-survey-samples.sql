SELECT TRIP_START_DATE,
 ...
 CASE WHEN SURVEY_SERIES_ALT_DESC IS NULL THEN SURVEY_SERIES_TYPE_ALT_DESC
  ...
  SP.SPECIMEN_ID,
  SM.SAMPLE_ID,
  CASE WHEN DNA.COLLECTED_ATTRIBUTE_CODE = 4 THEN DNA.STORAGE_CONTAINER_TYPE_CODE ELSE 'none' END AS DNA_SAMPLE_TYPE,
  CASE WHEN DNA.COLLECTED_ATTRIBUTE_CODE = 4 THEN DNA.STORAGE_CONTAINER_SUB_ID ELSE 'none' END AS DNA_CONTAINER_ID,
...
FROM GFBioSQL.dbo.SURVEY S
  ...
  INNER JOIN GFBioSQL.dbo.SPECIMEN_COLLECTED DNA ON SP.SPECIMEN_ID = DNA.SPECIMEN_ID AND SP.SAMPLE_ID = DNA.SAMPLE_ID
