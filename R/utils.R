...
fe_C <- fe_C_w_minor %>%
  ...
  dplyr::distinct() %>%
  mutate(skate_id = FE_PARENT_EVENT_ID) %>% ungroup()
...
final_event_counts <- sub_event_counts %>% filter(!is.na(fishing_event_id)) %>%
...
fe2 <- fe_A_no_parent %>% rename(fishing_event_id = FISHING_EVENT_ID) %>%
