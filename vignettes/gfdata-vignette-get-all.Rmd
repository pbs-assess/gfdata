...
d0 <- d0 |> rename(ssid = survey_series_id) |> 
  relocate(fishing_event_id, catch_count, catch_weight, ssid, survey_abbrev, year) |> 
  arrange(-catch_count) 
head(d0)
```
...
d <- d |> rename(ssid = survey_series_id) |> 
  relocate(fishing_event_id, skate_id, catch_count, catch_weight, ssid, survey_abbrev, year) |> 
  arrange(-catch_count) 
head(d)
```
