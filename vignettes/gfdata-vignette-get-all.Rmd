...
```{r, eval=TRUE, warning=TRUE}
d0 <- d0 |> 
  relocate(fishing_event_id, catch_count, catch_weight, survey_series_id, survey_abbrev, year) |> 
  arrange(-catch_count) 
head(d0)
```
...
```{r, eval=TRUE, warning=TRUE}
d <- d |> 
  relocate(fishing_event_id, skate_id, catch_count, catch_weight, survey_series_id, survey_abbrev, year) |> 
  arrange(-catch_count) 
head(d)
```
