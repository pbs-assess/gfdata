...
The `get_all_*()` functions have been designed to retrieve all fishery-independent survey data relevant to a particular species, or set of species, and to do so more quickly and comprehensively than previous functions. 
If retrieving data for multiple species at once, these functions will be a lot faster than the original `get_*()` functions because sql scripts are called once rather than repeatedly for each species. 
The extent of the data returned can be specific to a single survey, a single major stat area, any combination of these, or everything in the database. 
...
The original functions were limited in which survey series ids (`ssid`) they could accept and return. 
For `get_all*()` functions, we have the option to set `ssid = NULL` which will return all fishery-independent data in the database as long as a survey series id has been assigned (and for sets a survey id as well).
...
The original `get_*()` survey functions were limited to returning sets and specimen samples that conformed to current survey design specifications based on assigned grouping and usability codes.
...
While this behaviour can be reproduced reliably using the `get_all_*()` functions for some suveys including synoptic trawl (`ssid = c(1, 3, 4, 16)`) by using the options `grouping_only = TRUE` and `usability = c(0, 1, 2, 6)`, a small proportion of the data returned by the original function is missed when these options are applied to certain surveys (`ssid = c(###)`) and no data is returned for surveys for which these codes have not been generated (e.g., `ssid = c(###)`).
In contrast, the default behaviour of these functions is to return all data collected on any given survey, whether or not it conforms to current design. 
