...
```{r, echo = FALSE}
if (file.exists(file.path("pcod-cache", "pacific-cod.rds"))) { # quick check for speed
...
```{r, echo = FALSE}
if (file.exists(file.path("pcod-cache", "pacific-cod.rds"))) {
  dat  <- readRDS(file.path("pcod-cache", "pacific-cod.rds"))
  dat <- dat$survey_samples
  dat <- dat %>% filter(survey_series_id == 1)
 } else dat <- get_survey_samples(222, ssid = 1)
head(dat)
```

Or if you've already extracted the full survey sample dataset for Pacific Cod,
we could filter the full dataset for the desired survey series:
...
if (file.exists(file.path("pcod-cache", "pacific-cod.rds"))) {
  ...
  dat <- dat$survey_samples
  dat <- dat %>% filter(survey_series_id == 1)
 } else dat <- get_survey_samples("pacific cod", ssid = 1)
head(dat)
```

or on other variables:
